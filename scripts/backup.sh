#!/bin/bash
# ANERIUM OnePass — Automated Backup Script
...
# 2. Verify backup integrity (gzip test)
if ! gzip -t "$DB_BACKUP" 2>/dev/null; then
  log "ERROR: DB backup gzip integrity check failed"
  exit 1
fi
log "DB backup integrity verified (gzip test passed)"

# 3. Code + config backup (excluding node_modules, .git, public)
...
if [ ! -s "$CODE_BACKUP" ]; then
  log "WARNING: Code backup is empty — check tar command"
else
...
log "Pruning backups older than $RETENTION_DAYS days (before $PRUNE_DATE)"
